...
public class MsgAtMe
{
	...
	public MsgAtMe(Context pContext, RelativeLayout pSmsLayout, RelativeLayout pRelativeLayout, LinearLayout pMsgDetaiLayout)
	{

		mContext = pContext;
		mSmsLayout = pSmsLayout;
		mRelativeLayout = pRelativeLayout;
		mMsgDetailLayout = pMsgDetaiLayout;
		...
		mNewsList = (ListView) mRelativeLayout.findViewById(R.id.sms_msg_atme_list);
	...
	private void setListView()
	{
		...
		mNewsList.setOnItemClickListener(new OnItemClickListener()
		{
			...
			{
				mSmsLayout.removeAllViews();
				mSmsLayout.addView(mMsgDetailLayout);
		...
		mLoadMoreBtn = (Button) mRelativeLayout.findViewById(R.id.sms_msg_atme_loadMore_btn);
	...
	private class LoadNewsAsyncTask extends AsyncTask<Object, Integer, Integer>
	{
		...
		protected void onPostExecute(Integer result)
		{
			...
			{
			case NONEWS:
				Toast.makeText(mContext, "您还没有收到过消息", Toast.LENGTH_SHORT).show();
				break;
			case NOMORENEWS:
	...
	private int getSpeCateNews(List<HashMap<String, String>> newsList, int startnid, Boolean firstTimes)
	{
		...
		String params = "action=allDataAtMe" + "&userName=" + userName + "&startnid=" + startnid + "&count=" + NEWSCOUNT;
